    ...
    *)
    ...
    let ``01 Finding the length of a list, the hard way`` () =
        let length (xs : 'a list) : int =
           let rec inlist xs mylength =
              match xs with
              | [] -> mylength
              | head::tail -> inlist tail mylength+1
           inlist xs 0// write a function to find the length of a list
    ...
    let ``03 Reversing a list, the hard way`` () =
        let rev (xs : 'a list) : 'a list =
            let rec inlist xs out  =
               match xs with 
               | [] -> out
               | head::tail -> inlist (tail) (head::out)
            inlist xs [] 
    ...
    let ``05 Fixed-function mapping, the hard way (part 1).`` () =
        let map (xs : int list) : int list =
            let rec innerlist xs out =
              match xs with
              | [] -> out
              | head::tail -> innerlist (tail) ((head+1)::out)
            List.rev (innerlist xs []) 
    ...
    let ``06 Fixed-function mapping, the hard way (part 2).`` () =
        let map (xs : int list) : int list =
             let rec innerlist xs out =
               match xs with
               | [] -> out
               | head :: tail -> innerlist (tail) ((head*2)::out)
             List.rev (innerlist xs [])
    ...
    let ``07 Specified-function mapping, the hard way`` () =
        let map (f : 'a -> 'b) (xs : 'a list) : 'b list =
            let rec innerlist xs out =
               match xs with
               | [] -> out
               | head::tail -> innerlist (tail) (( f (head))::out)
            List.rev (innerlist xs [])
        ...
        let filter (f : 'a -> bool) (xs : 'a list) : 'a list =
            let rec innerlist xs out =
              ...
              | head::tail -> 
                              let applyf = f head
                              match applyf with
                              | true -> innerlist (tail) (head::out)
                              | false -> innerlist (tail) (out)
            List.rev (innerlist xs [])
        ...
        let filter (xs : int list) : int list =
             ...
             let rec innerlist xs out =
                ...
                | head::tail -> match head%2 with
                                | 1 -> innerlist (tail) (head::out)
                                | 0 -> innerlist tail out
...
*)
    ...
    let ``14 A fold which sums a list`` () =
        let fold initialState xs =
            ...
            let rec innerlist xs state =
               match xs with
               | [] -> state
               | head::tail -> innerlist (tail) (head+state)
            innerlist xs initialState
    ...
    let ``16 Folding, the hard way`` () =
        let fold (f : 'a -> 'b -> 'a) (initialState : 'a) (xs : 'b list) : 'a =
            ...
            let rec inlist xs state =
               match xs with 
               | [] -> state
               | head::tail -> let  innerfun = f (state) (head)
                               inlist (tail) (innerfun)
            (inlist (xs) (initialState))
    ...
    let ``18 exists: finding whether any matching item exists`` () =
        let exists (f : 'a -> bool) (xs : 'a list) : bool =
            ...
            let rec innerlist xs =
               match xs with
               | [] -> false
               | head::tail -> match (f head) with
                               | true -> true
                               | false -> innerlist tail 
            innerlist xs 
    ...
    let ``19 partition: splitting a list based on a criterion`` () =
        let partition (f : 'a -> bool) (xs : 'a list) : ('a list) * ('a list) =
             ...
             let rec innerlist xs even odd =
               match xs with 
               | [] -> List.rev (even), List.rev odd
               | head::tail -> match f (head) with
                               | true -> innerlist (tail) (head::even) (odd)
                               | false ->  innerlist (tail) (even) (head::odd)
             innerlist xs [] []
    ...
    let ``20 init: creating a list based on a size and a function`` () =
        let init (n : int) (f : int -> 'a) : 'a list =
            let rec innerlist i outlist=
              match i < n with 
              | true -> innerlist (i+1) ((f i)::outlist) 
              | false -> outlist
            List.rev (innerlist 0 [])
        ...
        let tryFind (p : 'a -> bool) (xs : 'a list) : 'a option =
            let rec list xs =
                           ...
                           | head::tail ->
                               ...
                               | false -> list tail
                               | true -> Some head
            list xs // Does this: https://msdn.microsoft.com/en-us/library/ee353506.aspx
    ...
    let ``22 tryPick: find the first matching element, if any, and transform it`` () =
        let tryPick (p : 'a -> 'b option) (xs : 'a list) : 'b option =
            let rec myf xs =
               match xs with 
               | [] ->  None      // Does this: https://msdn.microsoft.com/en-us/library/ee353814.aspx
               | head:: tail ->
                               match p head with
                               | Some _ -> (p head)
                               | None -> myf tail 
    ...
    *)
    ...
    let ``23 choose: find all matching elements, and transform them`` () =
        ...
        let choose (p : 'a -> 'b option) (xs : 'a list) : 'b list =
           ...
           let rec myf xs out =
             match xs with
             | [] -> out
             | head::rest -> match p head with
                             | Some c -> myf (rest)  (c::out)
                             | None -> myf rest out
           List.rev(myf xs [])
    ...
    let ``24 mapi: like map, but passes along an item index as well`` () =
        let mapi (f : int -> 'a -> 'b) (xs : 'a list) : 'b list =
             let rec inlist xs out index  =
                match xs with
                | [] -> out 
                | head::tail -> let fResult=  f (index) head
                                inlist tail (fResult::out) (index+1)
             List.rev (inlist xs [] 0)
